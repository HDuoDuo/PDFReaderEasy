...
//
//  Created by James Kingdom on 2018/01/22.
...
- (void)drawKnobs:(PDFDocumentCropOverlayKnob *)knob
{
    CGFloat x = knob.center.x;
    CGFloat y = knob.center.y;
    CGFloat w = knob.frame.size.width;
...
- (void)knobs:(PDFDocumentCropOverlayKnob *)knob moveTo:(CGPoint)point
{
    CGFloat x = point.x;
    CGFloat y = point.y;
    CGFloat w = knob.frame.size.width;
...
- (void)setCropRects:(CGRect)cropRect
{
    if (CGRectEqualToRect(cropRect, CGRectZero)) {
        self.knobRect = self.bounds;
    } else {
